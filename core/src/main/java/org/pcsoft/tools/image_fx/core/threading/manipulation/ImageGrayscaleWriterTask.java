 ...
 */
public final class ImageGrayscaleWriterTask extends MaskedImageWriterTask {

    public ImageGrayscaleWriterTask(int width, int height, PixelReader pixelReader, PixelWriter pixelWriter, Image maskImage, double opacity) {
        super(width, height, pixelReader, pixelWriter, maskImage, opacity);
    }

    private ImageGrayscaleWriterTask(int startX, int endX, int startY, int endY, ImageInformation imageInformation, PixelReader pixelReader, PixelWriter pixelWriter, boolean splitted, Image maskImage, double opacity) {
    ...
    protected Color _calculateColor(int x, int y, PixelReader pixelReader) throws ImageFXColorCalculatorException {
        return pixelReader.getColor(x, y).grayscale();
    }
    ...
    protected ImageWriterTask createTask(int startX, int endX, int startY, int endY, PixelReader pixelReader, PixelWriter pixelWriter, boolean splitted) {
        return new ImageGrayscaleWriterTask(startX, endX, startY, endY, imageInformation, pixelReader, pixelWriter, splitted, maskImage, opacity);
    }
}
