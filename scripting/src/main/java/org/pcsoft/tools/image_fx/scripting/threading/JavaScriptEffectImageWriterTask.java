 ...
 */
public final class JavaScriptEffectImageWriterTask extends JavaScriptImageWriterTask {

    public JavaScriptEffectImageWriterTask(int width, int height, PixelReader pixelReader, PixelWriter pixelWriter, ImageScriptElement scriptInfo, ImageScriptVariant variantInfo, Image maskImage, double opacity) {
        super(width, height, pixelReader, pixelWriter, scriptInfo, variantInfo, maskImage, opacity);
    }

    public JavaScriptEffectImageWriterTask(int startX, int endX, int startY, int endY, ImageInformation imageInformation, PixelReader pixelReader, PixelWriter pixelWriter, boolean splitted, ImageScriptElement scriptInfo, ImageScriptVariant variantInfo, Image maskImage, double opacity) {
    ...
    protected Color _calculateColor(int x, int y, PixelReader pixelReader) throws ImageFXColorCalculatorException {
        try {
            return JavaScriptManager.runEffectScript(scriptInfo, variantInfo,
                    imageInformation.getWidth(), imageInformation.getHeight(), x, y, pixelReader);
    ...
    protected ImageWriterTask createTask(int startX, int endX, int startY, int endY, PixelReader pixelReader, PixelWriter pixelWriter, boolean splitted) {
        return new JavaScriptEffectImageWriterTask(startX, endX, startY, endY, imageInformation, pixelReader, pixelWriter, splitted, scriptInfo, variantInfo, maskImage, opacity);
    }
